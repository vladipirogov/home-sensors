...
CCS811::CCS811(int nwake, int slaveaddr, TwoWire *theWire) {
  _nwake= nwake;
  ...
  wake_up();
    ...
    if( !ok ) {
      ...
      if( ok ) {
        ...
        printf("ccs811: wrong slave address, ping successful on other address\n");
      } else {
        PRINTLN(F("ccs811: ping failed (VDD/GND connected? SDA/SCL connected?)"));
        printf("ccs811: ping failed (VDD/GND connected? SDA/SCL connected?)\n");
      }
      goto abort_begin;
    }
